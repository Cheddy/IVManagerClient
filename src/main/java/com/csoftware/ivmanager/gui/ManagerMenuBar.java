...
public class ManagerMenuBar extends MenuBar implements ManagerEventHandlers {
	...
	private void initaliseFileMenu() {
		...
		if ((Constants.userPermissions & Constants.VIEW_STAFF_PERMISSION) != 0) {
		...
		if ((Constants.userPermissions & Constants.VIEW_WARD_PERMISSION) != 0) {
			MenuItem viewStaffRanks = new MenuItem("View Wards");
			viewStaffRanks.setAccelerator(new KeyCodeCombination(KeyCode.W, KeyCombination.CONTROL_DOWN));
			viewStaffRanks.setOnAction(VIEW_WARDS);
			fileMenu.getItems().add(viewStaffRanks);
		}
		if ((Constants.userPermissions & Constants.EDIT_INTERVENTION_PERMISSION) != 0) {
			MenuItem viewStaffRanks = new MenuItem("New Intervention");
			viewStaffRanks.setAccelerator(new KeyCodeCombination(KeyCode.N, KeyCombination.CONTROL_DOWN));
			viewStaffRanks.setOnAction(NEW_INTERVENTION);
			fileMenu.getItems().add(viewStaffRanks);
		}
